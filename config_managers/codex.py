...
class CodexConfigManager(BaseConfigManager):
    AGENT_NAME = "Codex"

    def __init__(self):
        home = os.path.expanduser("~")
        self.codex_dir = os.path.join(home, ".codex")
        self.agents_dir = os.path.join(home, ".agents")
        self.config_file = os.path.join(self.codex_dir, "config.json")
        self.agents_config_file = os.path.join(self.agents_dir, "config.json")
        self.skills_dir = os.path.join(self.codex_dir, "skills")
        self.agents_skills_dir = os.path.join(self.agents_dir, "skills")

    def _get_active_config_path(self) -> str:
        if os.path.exists(self.config_file):
            return self.config_file
        if os.path.exists(self.agents_config_file):
            return self.agents_config_file
        return self.config_file

    def is_installed(self) -> bool:
        """Detects if Codex or .agents environment is installed or present on the system."""
        if os.path.exists(self.codex_dir) and os.path.isdir(self.codex_dir):
            return True
        if os.path.exists(self.agents_dir) and os.path.isdir(self.agents_dir):
            return True
        if os.path.exists(self.config_file) or os.path.exists(self.agents_config_file):
            return True
    ...
    def save_mcp(self, mcp: McpServer) -> bool:
        ...
        mcp_dict = mcp.to_codex_dict() if hasattr(mcp, 'to_codex_dict') else mcp.to_claude_dict()
    ...
    def shelve_mcp(self, mcp: McpServer) -> bool:
        """Temporarily removes MCP from Codex config and stores in sidecar shelved file."""
        self.delete_mcp(mcp.name)
        return self.write_shelved_mcp(self._get_shelved_path(), mcp)

    def unshelve_mcp(self, mcp: McpServer) -> bool:
        """Restores a shelved MCP back into the active Codex config."""
    ...
    def list_plugins_and_skills(self, project_path: Optional[str] = None) -> List[PluginSkill]:
        ...
        skill_dirs = [
            (self.skills_dir, "~/.codex/skills"),
            (self.agents_skills_dir, "~/.agents/skills")
        ]
        seen_skills = set()
    ...
    def add_skill(self, name: str, description: str, instructions: str) -> bool:
        target_dir = self.skills_dir if os.path.exists(self.codex_dir) else self.agents_skills_dir
